...
first list if for walls to the N, S, E, W 
...
> makeMaze (x, y) walls = Maze (x, y) (map fst (filter isNorth walls) ++ [(i, y - 1) | i <- [0..x-1]] ++ map (reflect S) (map fst (filter isSouth walls) ++ [(i, 0) | i <- [0..x-1]])) 
>                                     (map fst (filter isSouth walls) ++ [(i, 0) | i <- [0..x-1]] ++ map (reflect N) (map fst (filter isNorth walls) ++ [(i, y - 1) | i <- [0..x-1]])) 
>                                     (map fst (filter isEast walls) ++ [(x - 1, j) | j <- [0..y-1]] ++ map (reflect W) (map fst (filter isWest walls) ++ [(0, j) | j <- [0..y-1]])) 
>                                     (map fst (filter isWest walls) ++ [(0, j) | j <- [0..y-1]] ++ map (reflect E) (map fst (filter isEast walls) ++ [(x - 1, j) | j <- [0..y-1]])) 
